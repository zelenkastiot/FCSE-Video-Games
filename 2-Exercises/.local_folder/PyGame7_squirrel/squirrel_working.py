...
CAMERASLACK_X = 90     # how far from the center the squirrel moves before moving the camera (along x axis)
CAMERASLACK_Y = 85     # how far from the center the squirrel moves before moving the camera (along y axis)
...
def runGame():
    ...
    first = {'surface': pygame.transform.scale(L_SQUIR_IMG, (STARTSIZE, STARTSIZE)),
                 'facing': LEFT,
                 'size': STARTSIZE,
                 'x': HALF_WINWIDTH,
                 'y': HALF_WINHEIGHT,
    ...
    second = {'surface': pygame.transform.scale(L_SQUIR_IMG, (STARTSIZE, STARTSIZE)),
             'facing': LEFT,
             'size': STARTSIZE,
             'x': HALF_WINWIDTH,
             'y': HALF_WINHEIGHT,
    ...
    third = {'surface': pygame.transform.scale(L_SQUIR_IMG, (STARTSIZE, STARTSIZE)),
             'facing': LEFT,
             'size': STARTSIZE,
             'x': HALF_WINWIDTH,
             'y': HALF_WINHEIGHT,
    ...
    while True: # main game loop
        ...
        # first: x + 120, y
        first['rect'] = pygame.Rect((first['x'] - camerax + 120,
                                         first['y'] - cameray - getBounceAmount(first['bounce'], BOUNCERATE,
                                                                                    BOUNCEHEIGHT),
                                         first['size'],
                                         first['size']))
        DISPLAYSURF.blit(first['surface'], first['rect'])

        # second: x + 120, y + 120
        second['rect'] = pygame.Rect((second['x'] - camerax + 120,
                                     second['y'] - cameray + 120 - getBounceAmount(second['bounce'], BOUNCERATE,
                                                                            BOUNCEHEIGHT),
                                     second['size']+20,
                                     second['size']))
        DISPLAYSURF.blit(second['surface'], second['rect'])

        # third: x - 150, y - 210
        third['rect'] = pygame.Rect((third['x'] - camerax - 150,
                                     third['y'] - cameray - 210 - getBounceAmount(third['bounce'], BOUNCERATE,
                                                                            BOUNCEHEIGHT),
                                     third['size']+10,
                                     third['size']))
        DISPLAYSURF.blit(third['surface'], third['rect'])
...
def makeNewSquirrelBegin(camerax, cameray, multiplier=1):
    sq = {}
    generalSize = random.randint(5, 25)
    # multiplier = random.randint(1, 3)
    sq['width']  = (generalSize + random.randint(0, 10)) * multiplier
